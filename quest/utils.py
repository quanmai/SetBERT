...
TEMPLATE_MAPPING = {
    '_': [],                           
    '_ or _': ['_ that are not _', '_ that are also _'],
    '_ that are not _': ['_ or _', '_ that are also _'],            
    '_ that are also _': ['_ or _', '_ that are not _'],
    '_ or _ or _': ['_ that are also _ but not _', '_ that are also both _ and _'],     
    '_ that are also _ but not _': ['_ or _ or _', '_ that are also both _ and _'],
    '_ that are also both _ and _': ['_ or _ or _', '_ that are also _ but not _'],
}
...
def generate_negative_query(original_query: str, template: str):
    # print(f"processing {original_query} with template {template}")
    # to generate negative query
    pattern = r'<mark>(.*?)</mark>'
    qlist = re.findall(pattern, original_query)
    if template == "_":
        return ["not " + qlist[0]] #qlist
    num_atoms = len(qlist)
    assert num_atoms > 0
    queries = []
    for tem in TEMPLATE_MAPPING[template]:
        query = qlist[0]
        pad = [t for t in tem.split('_') if t.strip()]
        assert len(pad) == num_atoms - 1
        for i in range(1, num_atoms):
            query += pad[i - 1] + qlist[i]
        queries.append(query)
    # add atoms query
    for atom_query in qlist:
        queries.append(atom_query)
    return queries
